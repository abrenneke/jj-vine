...
async fn test_multiple_independent_stacks_dont_incorrectly_retarget() {
    ...
    let output = repo.binary(&["submit", "--tracked", "--dry-run", "-v"]);
    assert!(output.status.success(), "submit should succeed");
    let output = strip_ansi_escapes::strip_str(&String::from_utf8_lossy(&output.stdout));
