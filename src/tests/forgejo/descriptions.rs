...
use crate::{
    ...
    error::Result,
    forge::{Forge as _, MergeRequestLike as _, UpdateMergeRequestInfoOptions},
    tests::TestRepo,
};
    ...
    repo.forge()
        .update_merge_request_info(
            ...
            UpdateMergeRequestInfoOptions::builder()
                ...
                .current_title(pr.title().to_owned())
                .build(),
        )
        .await?;
...
#[tokio::test]
async fn sync_description() -> Result<()> {
    let repo = TestRepo::with_forgejo_remote();
...
#[tokio::test]
async fn sync_description_disabled() -> Result<()> {
    let repo = TestRepo::with_forgejo_remote();
