...
async fn test_pr_description_links_to_dependent_prs() -> Result<()> {
    ...
    assert!(pr_a.description().contains(&format!("#{}", pr.iid())));
    assert!(pr.description().contains(&format!("#{}", pr_a.iid())));
...
async fn test_user_content_preserved_on_resubmit() -> Result<()> {
    ...
    let new_desc = format!("{}\n\n{}", pr.description(), user_content);
    repo.forge()
        .update_merge_request_description(pr.iid().as_ref(), &new_desc)
        .await?;
...
async fn test_skip_update_when_description_unchanged() -> Result<()> {
    ...
    assert_eq!(initial_desc, pr_a_after.description());
